...
class Amazon:
    def __init(aws_access_key_id, aws_secret_access_key, region_name):
    ...
    def upload file(self, vault, description, path):
        response = self.glacier_client.upload_archive(vaultName=vault, archiveDescription=description, body=open(path, 'rb'))
        self.log.info('File "{0}" upload successfull. Archive id: "{1}"'.format(path, response['archiveId']))
        return response
