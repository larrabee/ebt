...
class Mysql:
    ...
    def exec_command(self, sql_command):
        if 'unix_socket' in self.params:
            if self.params['passwd'] is None:
                ...
                db = MySQLdb.connect(unix_socket=self.params['unix_socket'], user=self.params['user'], passwd=self.params['passwd'],
                                     charset='utf8')
        else:
            if self.params['passwd'] is None:
                db = MySQLdb.connect(host=self.params['host'], port=self.params['port'], user=self.params['user'], charset='utf8')
            else:
    ...
    def slave_status_to_file(self, file_name='slave_data'):
        ...
        slave_data_file.write(
            ...
            .format(slave_status['Master_Host'], slave_status['Master_Port'], slave_status['Master_User'],
